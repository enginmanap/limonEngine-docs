...
.. note::
    Registered :ref:`camera rigs <implementCameraAttachment>` (``CameraExtensionInterface``) participate in this contract too - they hold the same protected ``parameters`` member and provide ``getParameters()`` / ``setParameters()``.
